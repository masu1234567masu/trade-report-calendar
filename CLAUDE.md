...
**対策**：`loadCalendarData()`のスプレッドシートID未設定時のエラーは`showGlobalError()`で画面上部に出し、あわせて`#settings-details`を自動で開くようにした。新しい自動実行フロー(ログイン直後・初期化時など、ユーザーが操作していないのに走る処理)でエラーが起きうる箇所は、`log()`だけで済ませず`showGlobalError()`を使うか検討すること。
